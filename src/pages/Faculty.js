...
const Faculty = () => {
  ...
  const getFilteredBookings = () => {
    ...
    return facultyBookings.filter(booking => {
      ...
      if (tabValue === 0) {
        statusMatch = booking.status === 'pending';
      } else if (tabValue === 1) {
        statusMatch = booking.status === 'approved';
      } else if (tabValue === 2) {
      ...
      <Paper sx={{ mb: 4 }}>
        ...
        >
          <Tab 
            ...
            label={`Pending (${facultyBookings.filter(b => b.status === 'pending').length})`} 
            iconPosition="start"
          />
          <Tab 
            icon={<CheckCircleIcon />} 
            label={`Approved (${facultyBookings.filter(b => b.status === 'approved').length})`} 
            iconPosition="start"
          />
          <Tab 
            icon={<CancelIcon />} 
